...
_Snapshot date: 2026-08-22. Read-only handoff — no application/source files were modified to produce this._
...
- `e7a8e45` — *"Cascade-delete story editorial images on story delete"* (Phase 12A).
- Working tree is **0 ahead / 0 behind** `origin/main`. **All Phase-13 visual work below is UNCOMMITTED (local only).**

## 4. Git status — modified / untracked files
**Modified (tracked):**
```
assets/experience.css        build.js                      lib/reader.js
index.html                   explore-data.js               stories.js
search-index.json            data/entities.json            data/photos.json
data/site.json               data/stories.json             data/story-photos.json
story/214-the-blue-chair.html                story/303-following-the-trail-harlan-left-us.html
family/hal.html  family/harlan.html  family/marvin-yaffe.html
journey/childhood.html  journey/faith-traditions.html  journey/family-home.html
journey/humor.html  journey/loss-memory.html  journey/objects-places.html
place/the-den.html  place/toledo-ohio.html  object/the-blue-recliner.html  event/harlans-death.html
```
**Untracked (new):**
```
assets/story-photos/         story-photos/
story/304-the-red-suitcase.html          story/305-the-tattoo-that-wasnt.html
HANDOFF_HARLANS_LEGACY.md    (this file)
```

### Source vs. generated (important for the new maintainer)
- **Hand-edited SOURCE (the real Phase-13 work):** `build.js`, `lib/reader.js`, `assets/experience.css`, and a small CSS/markup edit in `index.html`.
- **Everything else in the list is GENERATED build output** — regenerated by `node build.js` (and `node lib/hydrate.js`): all `story/`, `family/`, `journey/`, `place/`, `object/`, `event/` HTML, `stories.js`, `explore-data.js`, `search-index.json`, `data/*.json` (hydrated from Supabase), and the `assets/story-photos/` + `story-photos/` image trees. **Do not hand-edit generated files** — change the source + templates and rebuild.

## 5. Committed vs. uncommitted
- **Committed & deployed** (`origin/main` @ `e7a8e45`): the production platform — Supabase+Vercel backend, CMS, publishing, Phase-12A reliability fixes (see §6).
- **Uncommitted (local working tree only):** the entire **Phase-13 "Lit Room" visual transformation**, including the Story-Reader Stage-1 work. **None of it is on GitHub or Vercel.** A handoff must copy the working tree (or the maintainer must re-derive it), OR you commit it first.
...
- **Live:** `harlanslegacy.com` (apex 308→ `www`), served by **Vercel** from `origin/main` (currently `e7a8e45`). **Production does NOT yet contain the Phase-13 reader work.**
...
- **To deploy Phase 13:** commit the working tree to `main` and push → Vercel auto-builds. (Owner approval pending; do not push without it.)
