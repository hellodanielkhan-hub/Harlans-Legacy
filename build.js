...
function storyPageHTML(s, site, graph, journey, journeys) {
  ...
  const paragraphs = [s.lead, ...(s.body || [])]
    .filter(Boolean)
    .map(p => `          <p>${p}</p>`)
    .join("\n\n");
    ...
    <div class="container">
      <div class="story-card reveal" id="hl-story" data-story-id="${s.id}" data-theme="${attr(s.theme)}" data-reading-time="${s.readingTime || 0}" data-url="${attr(s.url)}">
        <div class="story-stamp">
          ...
          <span class="meta">Story No. ${s.id} &middot; ${text(s.dateLong)} &middot; ${text(s.themeLabel)}</span>
  ...
  <div class="container">
    <div class="footer-grid">
      ...
      <div class="footer-col">
        <h4>Elsewhere</h4>
        <ul>
          <!-- TODO(Daniel): replace with the real Harlan's Legacy Facebook page URL -->
