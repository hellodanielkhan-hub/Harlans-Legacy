...
async function hydrate() {
  ...
  let photos = 0, storyPhotos = 0, listen = 0;
  ...
  // narration audio + timing assets → assets/listen/<id>/… (where the reader loads them)
  try { listen = await hydrateBucket(store.BUCKET_LISTEN, path.join(ROOT, "assets", "listen")); } catch (e) { console.warn("hydrate listen:", e.message); }

  console.log(`hydrate: ${stories.length} stories · ${photos} family originals · ${storyPhotos} editorial originals · ${listen} narration assets.`);
}
