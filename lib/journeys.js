...
const { resolvePool, resolveCover } = require("./discover-pools.js");   // curated photo pools for the living Discover covers
...
function buildJourneyData(site, stories, entities, graph) {
  ...
  const peopleById = {};
  ((entities && entities.family && entities.family.people) || []).forEach(p => { peopleById[p.id] = p; });
  assignCovers(journeys, peopleById);
  return journeys;
}

// Give every journey a distinct, appropriate cover:
//   1. the first photograph of its curated family pool (lib/discover-pools.js) —
//      the same photograph its homepage Discover card opens on;
//   2. otherwise a fresh person's face (so a portrait never repeats across cards);
//   3. otherwise a cinematic motif of its defining place / object / event, or a
//      warm gradient art panel in the topic's thread colour.
// (Before the curated pools were consulted here, a theme whose only people had
// already given their faces to earlier themes — Loss & Memory: Harlan and Hal —
// fell through to the art panel, so its page had no photograph at all.)
function assignCovers(journeys, peopleById) {
  const usedPerson = new Set();
  journeys.forEach(j => {
    const lead = resolveCover(j.key, peopleById || {});
    if (lead) { usedPerson.add(lead.person); j.cover = { kind: "photo", entry: lead }; }
  });
  journeys.forEach(j => {
    if (j.cover) return;
...
// A curated pool photograph (see lib/discover-pools.js): the face-centred square
// crop, or the uncropped derivative for group photographs, with its focal point.
function poolPhoto(entry, prefix, sizes) {
  const { person, it, kind, widths, pos } = entry;
  const dir = `${prefix}assets/photos/${person}/`;
  const set = ext => widths.map(w => `${dir}${it.id}.${kind}.${w}.${ext} ${w}w`).join(", ");
  const largest = widths[widths.length - 1];
  const f = pos ? { x: pos[0], y: pos[1] } : (it.focus || { x: 50, y: 42 });
  const dims = kind === "portrait" ? `width="${largest}" height="${largest}"` : `width="${it.width || largest}" height="${it.height || largest}"`;
  return `<picture><source type="image/webp" srcset="${attr(set("webp"))}" sizes="${sizes}">` +
    `<img src="${attr(dir + it.id + "." + kind + "." + largest + ".jpg")}" srcset="${attr(set("jpg"))}" sizes="${sizes}" ` +
    `${dims} style="object-position:${f.x}% ${f.y}%" alt="" loading="lazy" decoding="async"></picture>`;
}

// Cover media: a curated family photograph or a fresh face (framed on the hero,
// filled on cards), a cinematic motif of a place/object/event, or a warm
// gradient art panel — see assignCovers.
function coverMedia(j, prefix, sizes) {
  const c = j.cover || { kind: "art" };
  if (c.kind === "photo") return `<span class="jc jc-photo">${poolPhoto(c.entry, prefix, sizes)}</span>`;
...
// A "living" cover: the pool's first photograph rendered exactly like a static
// cover (as the visible frame), plus the rest of the pool as compact data —
// photo path stem, available square widths, focal point — that
// assets/discover-living.js reveals one by one. Nothing else is downloaded
// until the section approaches the viewport.
function livingCover(pool, prefix, sizes) {
  const seq = pool.map(({ person, it, kind, widths, pos }) => {
    const f = pos ? { x: pos[0], y: pos[1] } : (it.focus || { x: 50, y: 42 });
    const e = { d: `${prefix}assets/photos/${person}/${it.id}`, w: widths, f: [f.x, f.y] };
    if (kind !== "portrait") e.k = kind;
    return e;
  });
  const first = seq[0], kind = first.k || "portrait";
  const set = ext => first.w.map(w => `${first.d}.${kind}.${w}.${ext} ${w}w`).join(", ");
  const largest = first.w[first.w.length - 1];
  const it = pool[0].it;
  const dims = kind === "portrait" ? `width="${largest}" height="${largest}"` : `width="${it.width || largest}" height="${it.height || largest}"`;
  const pic = `<picture class="jl-frame is-on"><source type="image/webp" srcset="${attr(set("webp"))}" sizes="${sizes}">` +
    `<img src="${attr(`${first.d}.${kind}.${largest}.jpg`)}" srcset="${attr(set("jpg"))}" sizes="${sizes}" ` +
    `${dims} style="object-position:${first.f[0]}% ${first.f[1]}%" alt="" loading="lazy" decoding="async"></picture>`;
  return `<span class="jc jc-photo jc-living" data-living="${attr(JSON.stringify(seq))}">${pic}</span>`;
}

// `entities` (optional) gives the whole family photo archive for the curated
// living-cover pools (lib/discover-pools.js); without it every card keeps its
// static cover.
function renderDiscoverCards(journeys, prefix, entities) {
  const peopleById = {};
  ((entities && entities.family && entities.family.people) || []).forEach(p => { peopleById[p.id] = p; });
  const sizes = "(max-width:640px) 92vw, (max-width:960px) 44vw, 30vw";
  return journeys.map(j => {
    const people = j.people.slice(0, 3).map(p => text(p.name)).join(" &middot; ");
    const pool = resolvePool(j.key, peopleById);
    const cover = pool.length >= 2 ? livingCover(pool, prefix, sizes) : coverMedia(j, prefix, sizes);
    return `        <a class="topic-card reveal pace-quick" data-theme="${j.key}" style="--tab-color:var(--thread-${j.key})" href="${prefix}${attr(j.url)}">
          <span class="topic-cover">${cover}<span class="topic-scrim"></span></span>
