...
const previewCap = require("./preview-cap.js");
...
async function route(parts, method, body, actor) {
  ...
  // ----- Ready-generation PREVIEW (moderator-only; NO publication, NO mutation) -----
  // Narration storage is PRIVATE. /preview (moderator-gated) issues a short-lived
  // capability bound to story + generation; /preview-asset (fetched header-less by
  // the cinematic engine) requires it and 302-redirects to a short-lived signed URL.
  // Without a valid capability, Ready/unapproved audio is unreachable.
  ...
  if (method === "GET" && (action === "preview" || action === "preview-asset")) {
    ...
    if (action === "preview-asset") {
      // preview-asset/<gen>/<capability>/<file>: exact generation of THIS story + valid capability
      if (!/^gen_[0-9a-f]+$/.test(gen) || !versions.some(x => x.generationId === gen)) return { status: 404, body: { error: "Not found" } };
      if (!previewCap.verify(id, gen, parts[5])) return { status: 403, body: { error: "Preview link expired or invalid. Reopen the preview from Admin." } };
    }
    ...
    if (action === "preview") {
      const base = "/api/narration/" + id + "/preview-asset/" + gen + "/" + previewCap.issue(id, gen) + "/";
      ...
      return { status: 200, body: {
        ...
        base, audioSrc: base + "narration.mp3", capabilityTtlMs: previewCap.TTL_MS, label: "PREVIEW — NOT PUBLISHED", paragraphs,
    ...
    // preview-asset/<gen>/<capability>/<file>
    ...
    return { status: 302, redirect: await assets.signedUrl(id, "g/" + gen + "/" + file, 600) };  // → short-lived signed URL (private bucket)
  }
