...
// Short-lived URL for ONE generation asset (Ready or approved), for authorized
// Admin Preview only. Production: a Supabase signed URL on the PRIVATE bucket.
// Local dev: the loopback dev server's path.
async function signedUrl(storyId, name, expiresInS) {
  if (store.USE_SUPABASE) return await store.signedObjectUrl(store.BUCKET_LISTEN, String(storyId) + "/" + name, expiresInS || 600);
  return "/assets/listen/" + storyId + "/" + name;
}

module.exports = { writeAssets, publicBase, existsLocal, exists, localDir, signedUrl };
