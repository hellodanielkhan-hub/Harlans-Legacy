...
// When derivatives already exist we skip the (slow) sharp regeneration — but we
// must still repopulate the responsive-width metadata the site build depends on.
// Otherwise a manifest that arrived EMPTY (e.g. hydrated from Supabase, which
// never stores these arrays) leaves a real photo looking like it has none.
// Reconstruct the arrays from the derivative files actually on disk.
function derivativeWidths(outDir, photoId) {
  const portrait = [], full = [];
  let files;
  try { files = fs.readdirSync(outDir); } catch (e) { return { portrait, full }; }
  const pPre = photoId + ".portrait.", fPre = photoId + ".full.";
  for (const f of files) {
    if (!f.endsWith(".webp")) continue;                    // count each width once (webp only)
    if (f.slice(0, pPre.length) === pPre) { const w = parseInt(f.slice(pPre.length), 10); if (!isNaN(w)) portrait.push(w); }
    else if (f.slice(0, fPre.length) === fPre) { const w = parseInt(f.slice(fPre.length), 10); if (!isNaN(w)) full.push(w); }
  }
  portrait.sort((a, b) => a - b); full.sort((a, b) => a - b);
  return { portrait, full };
}
// Natural width/height from the ORIGINAL (same values regeneration would record),
// so the manifest is identical whether derivatives were generated or reused.
async function sourceDims(srcPath, existing) {
  let width = (existing && existing.width) || null, height = (existing && existing.height) || null;
  if (sharp && (!width || !height)) {
    try { const m = await sharp(srcPath).metadata(); width = m.width || width; height = m.height || height; } catch (e) {}
  }
  return { width, height };
}
  ...
  for (const personId of ids) {
    ...
    for (const file of files) {
      ...
      if (sharp && !fs.existsSync(sentinel)) {
        ...
        // Skip regeneration, but rebuild the width metadata from disk so the
        // manifest is never left empty (e.g. after a Supabase hydrate).
        const dw = derivativeWidths(path.join(OUT, personId), photoId);
        const dims = await sourceDims(path.join(SRC, personId, file), existing);
        item.width = dims.width; item.height = dims.height;
        item.portrait = dw.portrait.length ? dw.portrait : (existing.portrait || []);
        item.full = dw.full.length ? dw.full : (existing.full || []);
        item.focus = existing.focus || { x: 50, y: 38 };   // preserve the face crop
      }
      items.push(item);
    }
    ...
    for (const key of keys) {
      ...
      for (const file of files) {
        ...
        if (sharp && !fs.existsSync(sentinel) && !fs.existsSync(sentinel2)) {
          ...
          // Skip regeneration, but rebuild the width metadata from disk so a real
          // story photo is never rendered as "no photo" after a Supabase hydrate.
          const dw = derivativeWidths(path.join(STORY_OUT, key), photoId);
          const dims = await sourceDims(path.join(STORY_SRC, key, file), existing);
          item.width = dims.width; item.height = dims.height;
          item.portrait = dw.portrait.length ? dw.portrait : (existing.portrait || []);
          item.full = dw.full.length ? dw.full : (existing.full || []);
          item.focus = existing.focus || { x: 50, y: 50 };   // preserve the editor's focal point
        }
        items.push(item);
      }
      const order = (prev.items || []).map(it => it.file).filter(f => files.includes(f));
