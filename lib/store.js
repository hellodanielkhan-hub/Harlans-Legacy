...
// Does an object exist in a public bucket? (split-host safe: the API host can
// check assets written by a remote worker to Supabase, not just local fs.)
async function objectExists(bucket, objPath) {
  if (!USE_SUPABASE) return false;
  const base = (SUPA_URL || "").replace(/\/$/, "") + "/storage/v1/object/public/" + bucket + "/" + objPath;
  try { const r = await fetch(base, { method: "HEAD" }); return r.ok; } catch (e) { return false; }
}
...
module.exports = {
  ...
  uploadObject, removeObject, listAll, downloadObject, objectExists
};
