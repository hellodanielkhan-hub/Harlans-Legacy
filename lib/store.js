...
const BUCKET_LISTEN = "listen";      // narration audio + timing assets (public read; server-side writes)
...
// Does an object exist? Service-role listing, so it works for PRIVATE buckets too
// (split-host safe: the API host can see assets a remote worker finalized).
async function objectExists(bucket, objPath) {
  if (!USE_SUPABASE) return false;
  const i = objPath.lastIndexOf("/");
  const dir = i >= 0 ? objPath.slice(0, i) : "", name = i >= 0 ? objPath.slice(i + 1) : objPath;
  try {
    const { data, error } = await sb().storage.from(bucket).list(dir, { search: name, limit: 100 });
    return !error && (data || []).some(f => f.name === name);
  } catch (e) { return false; }
}
// Short-lived signed URL for one object in a PRIVATE bucket (server-side only).
async function signedObjectUrl(bucket, objPath, expiresInS) {
  const { data, error } = await sb().storage.from(bucket).createSignedUrl(objPath, expiresInS || 600);
  if (error) throw new Error("store.signedUrl(" + objPath + "): " + error.message);
  return data.signedUrl;
}

module.exports = {
  USE_SUPABASE, BUCKET_PHOTOS, BUCKET_STORY, BUCKET_LISTEN,
  ...
  getDoc, putDoc,                         // generic single-JSONB docs (e.g. narration_jobs)
  uploadObject, removeObject, listAll, downloadObject, objectExists, signedObjectUrl
};
